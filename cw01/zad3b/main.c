...
int main(int argc, char **argv) {
    #ifdef DLL
    void* handle = dlopen("./shared_library_dir/liblibrary.so", RTLD_LAZY);
    if (handle == NULL){
        printf("Handle to shared library not found. Error: %s\n", dlerror());
        exit(1);
    }

    main_table* (*create_main_table)(size_t) = dlsym(handle, "create_main_table");
    block* (*define_sequence)(char*) = dlsym(handle, "define_sequence");
    FILE* (*merge)(block*) = dlsym(handle, "merge");
    int (*pin_new_block)(main_table*, block*, FILE*) = dlsym(handle, "pin_new_block");
    size_t (*no_of_lines_in_block)(block*) = dlsym(handle, "no_of_lines_in_block");
    void (*delete_line)(main_table*, int, int) = dlsym(handle, "delete_line");
    void (*delete_block)(main_table*, int) = dlsym(handle, "delete_block");
    void (*delete_table)(main_table*) = dlsym(handle, "delete_table");
    void (*print_merged_files)(main_table*) = dlsym(handle, "print_merged_files");
    ...
    for (int i = 1; i < argc; i++) {
        ...
        else if (strcmp(arg, "show_number_of_lines_in_block") == 0){
            int block_ind = atoi(argv[++i]);
            printf("%ld\n", no_of_lines_in_block(table->blocks[block_ind]));
        }
        else{
